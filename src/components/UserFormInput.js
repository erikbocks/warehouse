...
function UserFormInput(props) {
    ...
    return (
        <div className={'w-screen h-20 flex flex-col items-center justify-evenly'}>
            ...
            <input className={'p-2 w-60 border border-solid rounded-xl border-slate-500 bg-gray-100'} type={type} placeholder={placeholder} name={name} onChange={props.handleInputChange} required></input>
        </div>
    )
}
